...
contract GlobalsAndUtility is ERC20, IGlobals {
    ...
    */
    ...
    uint256 private constant GEOS_PER_KEEPER = 10 ** uint256(8); // 1e8
    uint256 private constant KEEPER_PER_BTC = 1e4;
    uint256 private constant SATOSHIS_PER_BTC = 1e8;
    uint256 public constant GEOS_PER_SATOSHI = GEOS_PER_KEEPER / SATOSHIS_PER_BTC * KEEPER_PER_BTC;

    /* Time of contract launch (2019-12-03T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1700642474;
    ...
    /* Seed for WAAS Lobby */
    uint256 internal constant WAAS_LOBBY_SEED_KEEPER = 1e9;
    uint256 internal constant WAAS_LOBBY_SEED_GEOS = WAAS_LOBBY_SEED_KEEPER * GEOS_PER_KEEPER;
    ...
    uint256 private constant BPB_MAX_KEEPER = 150 * 1e6;
    uint256 internal constant BPB_MAX_GEOS = BPB_MAX_KEEPER * GEOS_PER_KEEPER;
    ...
    uint8 internal constant ETH_ADDRESS_KEEPER_LEN = ETH_ADDRESS_BYTE_LEN * 2;

    uint8 internal constant CLAIM_PARAM_HASH_BYTE_LEN = 12;
    uint8 internal constant CLAIM_PARAM_HASH_KEEPER_LEN = CLAIM_PARAM_HASH_BYTE_LEN * 2;

    uint8 internal constant BITCOIN_SIG_PREFIX_LEN = 24;
    bytes24 internal constant BITCOIN_SIG_PREFIX_STR = "Bitcoin Signed Message:\n";

    bytes internal constant STD_CLAIM_PREFIX_STR = "Claim_KEEPER_to_0x";
    bytes internal constant OLD_CLAIM_PREFIX_STR = "Claim_BitcoinKEEPER_to_0x";

    bytes16 internal constant KEEPER_DIGITS = "0123456789abcdef";
    ...
    function getGlobalsLoad(GlobalsCache memory g, GlobalsCache memory gSnapshot) external view{
    ...
    // Implementing the interface function
    function getTreasureBoxAddress() public view returns (address) {
        return treasureBoxAddress;
    }

    // Implementing the interface function
    function mint(address _receiver, uint256 _amount, address _contractAddress) external{
        require(_contractAddress == treasureBoxAddress, "Caller is not TB contract");
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "KEEPER: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "KEEPER: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "KEEPER: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "KEEPER: stakeIdParam not in stake");
