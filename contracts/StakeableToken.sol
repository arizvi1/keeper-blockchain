...
contract StakeableToken is GlobalsAndUtility {
     ...
     * @param newStakedGeos Number of Geos to stake
     * @param newStakedDays Number of days to stake
     */
    function stakeStart(uint256 newStakedGeos, uint256 newStakedDays)
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "KEEPER: newStakedDays lower than minimum");

        /* Check if log data needs to be updated */
        _dailyDataUpdateAuto(g);

        _stakeStart(g, newStakedGeos, newStakedDays, false);

        /* Remove staked Geos from balance of staker */
        _burn(msg.sender, newStakedGeos);
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "KEEPER: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "KEEPER: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "KEEPER: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "KEEPER: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "KEEPER: Empty stake list");
        require(stakeIndex < stakeListRef.length, "KEEPER: stakeIndex invalid");
            ...
            if (prevUnlocked) {
                ...
                if (servedDays > st._stakedDays) {
                    ...
                    if (servedDays < MIN_AUTO_STAKE_DAYS) {
                        require(!st._isAutoStake, "KEEPER: Auto-stake still locked");
                    }
                }
            }
        ...
        g._lockedGeosTotal -= st._stakedGeos;
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "KEEPER: newStakedDays higher than maximum");

        uint256 bonusGeos = _stakeStartBonusGeos(newStakedGeos, newStakedDays);
        uint256 newStakeShares = (newStakedGeos + bonusGeos) * SHARE_RATE_SCALE / g._shareRate;

        /* Ensure newStakedGeos is enough for at least one stake share */
        require(newStakeShares != 0, "KEEPER: newStakedGeos must be at least minimum shareRate");
        ...
        */
        ...
        _emitStakeStart(newStakeId, newStakedGeos, newStakeShares, newStakedDays, newAutoStake);

        /* Stake is added to total in the next round, not the current round */
        g._nextStakeSharesTotal += newStakeShares;

        /* Track total staked Geos for inflation calculations */
        g._lockedGeosTotal += newStakedGeos;
    }
        ...
        returns (uint256 payout)
    {
        ...
        if (beginDay <= BIG_PAY_DAY && endDay > BIG_PAY_DAY) {
            uint256 bigPaySlice = g._unclaimedSatoshisTotal * GEOS_PER_SATOSHI * stakeSharesParam
    ...
    /*
     * @dev Calculate bonus Geos for a new stake, if any
     * @param newStakedGeos Number of Geos to stake
     * @param newStakedDays Number of days to stake
     */
    function _stakeStartBonusGeos(uint256 newStakedGeos, uint256 newStakedDays)
        private
        pure
        returns (uint256 bonusGeos)
    {
        /*
            LONGER PAYS BETTER:
            ...
            Bonus percentage scaled 0% to 10% for the first 150M KEEPER of stake.

            biggerBonus%    = (cappedGeos /  BPB_MAX_GEOS) * 10%
                            = (cappedGeos /  BPB_MAX_GEOS) / 10
                            =  cappedGeos / (BPB_MAX_GEOS * 10)
                            =  cappedGeos /  BPB
            ...
            combinedBonus%  =            longerBonus%  +  biggerBonus%
                                      ...
                                      cappedExtraDays     cappedGeos
                            =         ---------------  +  ------------
                                            LPB               BPB

                                cappedExtraDays * BPB     cappedGeos * LPB
                            =   ---------------------  +  ------------------
                                      LPB * BPB               LPB * BPB
            ...
            bonusGeos     = geos * combinedBonus%
                            = geos * (cappedExtraDays * BPB  +  cappedGeos * LPB) / (LPB * BPB)
        */
        ...
        uint256 cappedStakedGeos = newStakedGeos <= BPB_MAX_GEOS
            ? newStakedGeos
            : BPB_MAX_GEOS;

        bonusGeos = cappedExtraDays * BPB + cappedStakedGeos * LPB;
        bonusGeos = newStakedGeos * bonusGeos / (LPB * BPB);
        ...
        returns (uint256 stakeReturn, uint256 payout, uint256 penalty, uint256 cappedPenalty)
    {
        if (servedDays < st._stakedDays) {
            ...
            stakeReturn = st._stakedGeos + payout;
        } else {
        ...
        private
    {
        if (stakeReturn > st._stakedGeos) {
            ...
            */
            uint256 bonusGeos = _stakeStartBonusGeos(stakeReturn, st._stakedDays);
            uint256 newShareRate = (stakeReturn + bonusGeos) * SHARE_RATE_SCALE / st._stakeShares;
