...
contract FoundersKeeper is Ownable, ERC20 {
    ...
    event FoundersKeeperBought(
        address buyer,
        uint256 _totalFkeeper,
        uint256 _tax,
        uint256 receivedFkeeper
    );
    event FoundersKeeperSold(
        address seller,
        uint256 _totalFkeeper,
        uint256 _tax,
        uint256 soldFkeeper
    ...
    constructor(
        ...
        address _teamTokens,
        address _tbg
    ) Ownable() ERC20("FOUNDERS KEEPER TOKEN", "FKEEPER") {
        ORIGIN_ADDRESS = payable(msg.sender);
        _mint(address(this), ONE_BILLION_ETHERS);
        _transfer(
            address(this),
            _privateSafe,
            calculateValueOnPercentage(ONE_BILLION_ETHERS, 10)
        ); //10% private safe
        _transfer(
            address(this),
            _teamTokens,
        ...
        _transfer(
            address(this),
            ORIGIN_ADDRESS,
            calculateValueOnPercentage(ONE_BILLION_ETHERS, 50)
        ); //50% sellable on DEX
    }
    ...
    function calculateValueOnPercentage(
        uint256 _amount,
        uint256 _percentage
    ...
    function setTreasureBox(address _treasureBox) public onlyOwner {
        ...
        if (TREASURE_BOX == address(0)) {
            ...
            treasureBoxAssests[_treasureBox] = calculateValueOnPercentage(
                ONE_BILLION_ETHERS,
                20
