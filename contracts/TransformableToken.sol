...
contract TransformableToken is UTXORedeemableToken {
    ...
    function xfLobbyEnter(address referrerAddr) external payable
    {
        ...
        // 1 < 351
        require(enterDay < CLAIM_PHASE_END_DAY, "KEEPER: Lobbies have ended");

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "KEEPER: Amount required");
    ...
    {
        require(enterDay < _currentDay(), "KEEPER: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "KEEPER: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "KEEPER: count invalid");
        }

        uint256 waasLobby = _waasLobby(enterDay); // Returns Amount of KEEPER that is available for distribution in the Transform Lobbies on a specific day
        ...
        do {
            ...
            /* 1. The formula for calculating the share of KEEPER 
               2. rawAmount: It is the amount of ETH that the user entered with.
               3. waasLobby: The amount of KEEPER that is available for distribution in the Transform Lobbies on the day that the user entered. 
    ...
    // {
    //     require(address(this).balance != 0, "KEEPER: No value");
    ...
    function xfLobbyEntry(address memberAddr, uint256 entryId) external view returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "KEEPER: Param invalid");
    ...
    /* This function is used to calculate the amount of KEEPER that is available for distribution in the Transform Lobbies on a specific day.*/
